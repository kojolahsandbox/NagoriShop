...
@section('content')
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Form Tambah Penjual</h3>
        </div>
        <div class="card-body">
...
@section('script')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Data alamat dari old input (jika ada error validasi)
            const oldData = {
                ...
                .then(data => {
                    ...
                    data.forEach(prov => {
                        provinsiSelect.innerHTML +=
                            `<option value="${prov.id}" ${prov.id == oldData.province ? 'selected' : ''}>${prov.name}</option>`;
                    });
                    if (oldData.province) provinsiSelect.dispatchEvent(new Event('change'));
                });
                    ...
                    .then(data => {
                        ...
                        data.forEach(kota => {
                            kotaSelect.innerHTML +=
                                `<option value="${kota.id}" ${kota.id == oldData.city ? 'selected' : ''}>${kota.name}</option>`;
                        });
                        if (oldData.city) kotaSelect.dispatchEvent(new Event('change'));
                    });
            });
                    ...
                    .then(data => {
                        ...
                        data.forEach(kec => {
                            kecamatanSelect.innerHTML +=
                                `<option value="${kec.id}" ${kec.id == oldData.district ? 'selected' : ''}>${kec.name}</option>`;
                        });
                        if (oldData.district) kecamatanSelect.dispatchEvent(new Event('change'));
                    });
            });
                    ...
                    .then(data => {
                        ...
                        data.forEach(kel => {
                            kelurahanSelect.innerHTML +=
                                `<option value="${kel.id}" ${kel.id == oldData.village ? 'selected' : ''}>${kel.name}</option>`;
                        });
                    });
            });
        });
